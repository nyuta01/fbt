...
func runAgent(req request) error {
	...
	for i, tool := range tools {
		...
		writeNotification("fbt/event", map[string]any{
			...
			"level":            "info",
			"message":          "local agent tool call completed",
	...
	writeNotification("fbt/event", map[string]any{
		...
		"event_type":       "usage",
		"level":            "info",
		"message":          "local agent runner completed deterministic plan",
		"attributes": map[string]any{
...
func agentContent(transform transformInfo, tools []string) string {
	...
	builder.WriteString("\n## Result\n\nGenerated by the local FBT agent runner.\n")
...
func provenanceFor(model map[string]any, tools []string) map[string]any {
	return map[string]any{
		"runner":                "fbt-local-agent-runner",
		"runner_version":        "0.1.0",
		"model_provider":        stringValue(model, "provider", "local"),
		"model":                 stringValue(model, "name", "mock-agent"),
