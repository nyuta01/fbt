...
`},
  ...
  - name: incident_timeline
    ...
    tags: ["incident"]
`},
		fileSpec{Path: "README.md", Content: "# " + projectName + "\n\nRun `fbt build --select incident_timeline` to generate the local incident timeline example.\n"},
	)
	return files
}
...
func runnableBaseFiles(projectName, runnerRoot, selectors string) []fileSpec {
	return []fileSpec{
		...
		{Path: "bin/fbt-local-llm-runner", Content: runnerScript(filepath.Join(runnerRoot, "runners", "llm")), Executable: true},
		{Path: "bin/fbt-local-agent-runner", Content: runnerScript(filepath.Join(runnerRoot, "runners", "agent")), Executable: true},
	}
}
