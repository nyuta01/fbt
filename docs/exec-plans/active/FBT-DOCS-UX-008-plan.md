...
preview, generate, inspect, approve, and explain. For each command, state the
...
offline quickstart output for build/review/history lifecycle signals. It closes
with the short mental model:
`plan` decides, `build` generates, `review` trusts, and `artifact history`
explains later.
