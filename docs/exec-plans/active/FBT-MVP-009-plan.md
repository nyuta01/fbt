...
- `adapters/command`: protocol runner that executes a configured local command
...
Added process-level Go tests that start `tests/runner_fixtures/fake` and `adapters/command`
