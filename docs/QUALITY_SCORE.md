...
| fbt Spec Coverage | 5 | Core, project config, manifest, state, runner protocol, schema/versioning, runner discovery, security/conformance, usage, and example specs exist | Eval/review specs still need implementation evidence | `FBT-MVP-012` |
...
| Runner Protocol | 4 | `internal/protocol` fake-runner tests cover initialize, runTransform, JSONL notifications, output candidates, JSON-RPC errors, cancellation, and build invocation | Capability validation remains shallow | `FBT-MVP-016` |
