...
|---|---:|---|---|---|
| Harness PDCA | 4 | `make verify` includes harness, drift, docs, Go, CLI smoke, e2e knowledge-loop smoke checks, and task-by-task commits | Product conformance tests are not implemented yet | `FBT-MVP-016` |
| fbt Spec Coverage | 5 | Core, project config, manifest, state, runner protocol, schema/versioning, runner discovery, security/conformance, usage, examples, init templates, diff/docs, and eval/review behavior exist | Release/conformance behavior still needs implementation evidence | `FBT-MVP-016` |
| Go CLI Scaffold | 4 | CLI now exposes help/version plus init, parse, plan, build, eval, review, diff, docs, state, artifact, and runner diagnostics with tests and smoke coverage | Packaging and conformance surfaces are still pending | `FBT-MVP-016` |
...
| Local Runners | 4 | `runners/fake`, `runners/command`, `runners/llm`, and `runners/agent` process-level tests verify protocol compatibility, output candidates, usage/provenance, tool-call events, local build use, and init-generated e2e flow without external services | Release packaging is still pending | `FBT-MVP-016` |
