...
| fbt Spec Coverage | 5 | Core, project config, manifest, state, runner protocol, schema/versioning, runner discovery, security/conformance, usage, examples, init templates, and eval/review behavior exist | Diff/docs-generation behavior still needs implementation evidence | `FBT-MVP-015` |
| Go CLI Scaffold | 4 | CLI now exposes help/version plus init, parse, plan, build, eval, review, state, artifact, and runner diagnostics with tests and smoke coverage | Diff and docs commands are still pending | `FBT-MVP-015` |
