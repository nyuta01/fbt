...
| Manifest Graph | 4 | `internal/manifest` and `internal/graph` now test canonical IDs, parent/child maps, deterministic JSON, and selectors | Manifest writing is not yet exposed through CLI | `FBT-MVP-006` |
...
| Local State | 4 | `internal/state` tests atomic snapshot writes, append-only run results, lock behavior, stale lock recovery, and immutable artifact version records | State is not yet integrated into planner/build CLI flows | `FBT-MVP-006` |
| Planner Baseline | 3 | `internal/planner` tests run/skip/blocked actions, dirty reasons, selected sets, and review/confidence blocking | Planner has no CLI surface and only a first-pass dirty reason model | `FBT-MVP-006` |
